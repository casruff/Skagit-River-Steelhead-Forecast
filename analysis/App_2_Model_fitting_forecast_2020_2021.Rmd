...
fit_jags2<-function(model,data,params,inits,ctrl,dir=jagsdir){
  jm<-jags.parallel(data=data, 
                    ...
                    n.burnin = ctrl$burn,
                    n.thin = ctrl$thin,
                    n.cluster= ctrl$chains, 
                    DIC = TRUE,
  )
  return(as.mcmc.list(jm))
}
...
model {
  ...
  for(t in 2:(n_yrs-age_min+n_fore)) {
    ...
    #=============================
    #version 1; fully non-centered
    #=============================
    #res_ln_Rec[t] ~ dnorm(0, 1);
    #w[t] <- phi * w[t-1] + theta_res * res_ln_Rec[t-1] * sigma_r + res_ln_Rec[t] * sigma_r;
    #=============================
    #version 2; partially centered (what you ran)
    #=============================
    #res_ln_Rec[t] ~ dnorm(0, tau_r);
    #w[t] <- phi * w[t-1] + theta_res * res_ln_Rec[t-1] + res_ln_Rec[t];
    #============================
    #version 3; fully centered
    #============================
    w[t] ~ dnorm(phi * w[t-1] + theta_res * res_ln_Rec[t-1], tau_r); 
    res_ln_Rec[t] <- w[t]-w[t-1];
...
my_sso <- launch_shinystan(as.shinystan(fit_bh_cov_AR1))
