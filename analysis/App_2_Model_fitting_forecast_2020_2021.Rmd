...
extract_log_lik<-function(m,esc_only,N){
...
approx_LFO<-function(N=N,L,m=m,esc_only){
  loglik =  extract_log_lik(m=m, esc_only = esc_only,N=N)
  ks <- NULL
  ...
  results<-list(approx_elpds_1sap_no_refit,ks)
  names(results)<-c("LFO","ks")
  return(results)
}
...
mod_fits[[1]] <- readRDS(file.path(savedir, "saved_models","fit_bh_cov_MA1_AR1.rds"))
mod_fits[[2]] <- readRDS(file.path(savedir, "saved_models","fit_bh_cov_AR.rds"))
mod_fits[[3]] <- readRDS(file.path(savedir, "saved_models", "fit_bh_cov_AR_resid.rds"))

L=20
N=yr_last-yr_frst
esc_only="No"
LFO1<-approx_LFO(N=N,L=L,m=1,esc_only=esc_only)
plot_ks(LFO1$ks,N=N,L=L)

LFO2<-approx_LFO(N=N,L=L,m=2,esc_only=esc_only)
plot_ks(LFO2$ks, N=N,L=L)

LFO3<-approx_LFO(N=N,L=L,m=3,esc_only=esc_only)
