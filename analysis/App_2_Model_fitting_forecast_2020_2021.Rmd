...
loadmodfits<-function(modelnames){
  ...
  for(i in 1:length(modelnames)){
    mod_fits[[i]] <- readRDS(file.path(savedir, "saved_models",paste0("fit_",modelnames[i],".rds")))
  }
  return(mod_fits)
}
...
approx_LFO<-function(N=N,L,m=m,esc_only,mod_fits,userefits,refitname,thres){
  ...
  results<-list(approx_elpds_1sap,ks)
...
LFO1<-approx_LFO(N=N,L=L,m=1,esc_only=esc_only,mod_fits=mod_fits,userefits="Yes",refitname="BH_cov_MA1_AR1",thres=0.7)
plot_ks(LFO1$ks,N=N,L=L)

LFO2<-approx_LFO(N=N,L=L,m=2,esc_only=esc_only,mod_fits=mod_fits,userefits="Yes",refitname="BH_cov_AR",thres=0.7)
plot_ks(LFO2$ks, N=N,L=L)

LFO3<-approx_LFO(N=N,L=L,m=3,esc_only=esc_only,mod_fits=mod_fits,userefits="Yes",refitname="BH_cov_AR_resid",thres=0.7)
