...
For the most recent 10 years (2016 - 2025), fit the model to data through year t-1 and generate a forecast for year t. 
...
pred_trs <- NULL
pred_trs_post <-  vector("list", n_forecasts)
for(n in 1:n_mods){
  #n <- 1
  pred_esc <- NULL
  for(i in 1:(n_forecasts)){
    ...
    p_dat_post <- mod_res[,grep("Sp", colnames(mod_res))]
    p_dat <- round(median(p_dat_post[,dim(p_dat_post)[2]]))

    p_dat_post <- p_dat_post[,dim(p_dat_post)[2]]
    pred_esc[i] <- p_dat

    pred_trs_post[[i]] <- cbind(pred_trs_post[[i]],p_dat_post) 
  }
...
# Model Averaging and 2025 - 2026 forecast
Here is weighted average forecast for 2025 - 2026 based on the model LFOIC weights. We
also compare predictions for return years 2016 - 2025 generated from each of the
six models evaluated including an ensemble (weighted) to observed run size.

```{r }
## extract median 2026 forecast from each model
f_dat<-data.frame(
...
model_selection[,"2026_forecast"] <- apply(f_dat,2,median)

weighted_forecast_dist <-(
  as.matrix(f_dat) %*% (as.vector(model_selection[,"LFOIC_weight"]))
)

# ensemble_forecast_posterior_retrospective <- cbind(as.matrix(pred_trs_post[[4]]) %*% (as.vector(model_selection[,"LFOIC_weight"])),as.matrix(pred_trs_post[[6]]) %*% (as.vector(model_selection[,"LFOIC_weight"])),as.matrix(pred_trs_post[[7]]) %*% (as.vector(model_selection[,"LFOIC_weight"])),as.matrix(pred_trs_post[[9]]) %*%
# (as.vector(model_selection[,"LFOIC_weight"])),as.matrix(pred_trs_post[[10]]) %*%                                 (as.vector(model_selection[,"LFOIC_weight"])),weighted_forecast_dist)
# colnames(ensemble_forecast_posterior_retrospective)<-c("ensemble_forecast_posterior_2017",
#                                                        "ensemble_forecast_posterior_2019",
#                                                        "ensemble_forecast_posterior_2020",
#                                                        "ensemble_forecast_posterior_2022",
#                                                        "ensemble_forecast_posterior_2023",
#                                                        "ensemble_forecast_posterior_2024")


# ensemble_forecast_posterior_retrospective <- data.frame(ensemble_forecast_posterior_retrospective)
...
# write.csv(ensemble_forecast_posterior_retrospective,file.path(savedir,"ensemble_forecast_posterior_retrospective.csv"),row.names = F)
...
plot(x=1,y=1,ylim=c(0,0.001),xlim=c(0,10000),ylab="density",xlab="2025 - 2026 Runsize")
