...
p_dat_AR1MA1_2025 <- round(quantile(p_dat_AR1MA1[,n_yrs+n_fore],probs = CI_vec))


p_dat_AR1 <- mod_res_AR1[,grep("Sp", colnames(mod_res_AR1))]
p_dat_AR1_2025 <- round(quantile(p_dat_AR1[,n_yrs+n_fore],probs = CI_vec))
...
tbl_LOOIC <- cbind(model = c("B-H","B-H"),
                   error = c("MA1_AR1","AR1"),
                   as.data.frame(tbl_LOOIC),pseudo_bma_weight = as.matrix(model_weights),forecast = c(p_dat_AR1MA1_2025[2],p_dat_AR1_2025[2]))
...
```{r fig_4_cov_effects, fig.width=8, fig.height=10, fig.pos="placeHere", warnings=FALSE, messages=FALSE, fig.align="left"}
clr <- rgb(0, 0, 255, alpha = 50, maxColorValue = 255)
xoffSet <- 0.04
yoffSet <- 0.03
...
```{r fig_7_ref_pts, fig.width=8, fig.height=4, warning=FALSE, fig.pos = "placeHere", fig.align="left"}
