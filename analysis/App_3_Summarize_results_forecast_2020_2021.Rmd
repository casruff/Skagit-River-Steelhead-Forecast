...
p_dat_MA1_Ar1 <- mod_res_MA1_AR1[,grep("Sp", colnames(mod_res_MA1_AR1))]
p_dat_MA1_Ar1 <- round(quantile(p_dat_MA1_Ar1[,n_yrs+n_fore],
                                   probs=c(0.025,0.25,0.5,0.75,0.975)))

p_dat_Ar1 <- mod_res_AR1[,grep("Sp", colnames(mod_res_AR1))]
p_dat_Ar1 <- round(quantile(p_dat_Ar1[,n_yrs+n_fore],
                                   probs=c(0.025,0.25,0.5,0.75,0.975)))
=======
...
tbl_LOOIC <- cbind(model = c("B-H","B-H"),
                   error = c("MA1_AR1","AR1"),
<<<<<<< Updated upstream
                   as.data.frame(tbl_LOOIC),pseudo_bma_weight = as.matrix(model_weights),forecast = c(p_dat_MA1_Ar1[3],p_dat_Ar1[3]))
tbl_LOOIC[order(tbl_LOOIC[,"looic"]), ]
write.csv(tbl_LOOIC,file.path(savedir,"model_selection_results.csv"))
=======
                   as.data.frame(tbl_LOOIC),pseudo_bma_weight = as.matrix(model_weights),forecast_2020 = c(p_dat_AR1MA1_2020[2],p_dat_AR1_2020[2]))
tbl_LOOIC[order(tbl_LOOIC[,"looic"]),]

write.csv(tbl_LOOIC,file.path(savedir,"Model_summary_table.csv"))
...
data.frame(forecast=round(quantile(p_dat[,n_yrs+n_fore],
                                   probs=c(0.025,0.25,0.5,0.75,0.975))))
=======
data.frame(forecast=round(quantile(p_dat[,n_yrs+n_fore],                                   probs=c(0.025,0.25,0.5,0.75,0.975))))
>>>>>>> Stashed changes
```
...
## plot data
p_dat <- mod_res_MA1_AR1[,grep("res_ln_Rec", colnames(mod_res_MA1_AR1))]*model_weights[1] + mod_res_AR1[,grep("res_ln_Rec", colnames(mod_res_AR1))]*model_weights[2] 
