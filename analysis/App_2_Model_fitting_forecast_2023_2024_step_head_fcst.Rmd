...
For the most recent 10 years (2014 - 2023), fit the model to data through year t-1 and generate a forecast for year t. 
...
# Model Averaging and 2023 - 2024 forecast
...
model_selection[,"2024_forecast"] <- apply(f_dat,2,median)

weighted_forecast_dist <-(
  as.matrix(f_dat) %*% (as.vector(model_selection[,"LFOIC_weight"]))
)

ensemble_forecast_posterior_retrospective <- cbind(as.matrix(pred_trs_post[[5]]) %*% (as.vector(model_selection[,"LFOIC_weight"])),as.matrix(pred_trs_post[[7]]) %*% (as.vector(model_selection[,"LFOIC_weight"])),as.matrix(pred_trs_post[[8]]) %*% (as.vector(model_selection[,"LFOIC_weight"])),as.matrix(pred_trs_post[[10]]) %*% (as.vector(model_selection[,"LFOIC_weight"])),weighted_forecast_dist)
colnames(ensemble_forecast_posterior_retrospective)<-c("ensemble_forecast_posterior_2018",
                                                       "ensemble_forecast_posterior_2020",
                                                       "ensemble_forecast_posterior_2021",
