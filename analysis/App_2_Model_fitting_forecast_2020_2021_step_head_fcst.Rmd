      ...
      t <- t + 1
        ...
        if(model == "IPM_BH_cov_AR"|model == "IPM_BH_cov_AR_resid"){
          ...
          par_jags<- c("alpha","E_BH_a","ln_BH_a",
                        "beta",
                        "gamma",
                        "Sp","Run","Rec","tot_ln_Rec","ln_RS","tot_Run",
                        "pi_eta","pi_tau",
...
for(n in 1:n_mods){
  ...
  mod_res_pred[,grep("^Run", colnames(mod_res_pred))]
  ...
  p_dat_obs_adj <- cbind(((mod_res_obs[,paste0("Run","[",n_yrs - age_min,",",1,"]")])),
  ...
  pred_trs_mod_adj <- pred_esc_adj + 1
...
=======
matplot(as.matrix(pred_trs),type="l",lty=c(1:3),col="black",lwd=2,ylab="Catch + Escapement",xaxt="n",ylim=c(min(pred_trs,obs_trs),max(pred_trs,obs_trs)))
>>>>>>> Stashed changes
